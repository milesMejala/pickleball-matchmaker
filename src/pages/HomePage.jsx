...
export default function HomePage() {
  ...
  function removePlayer(id) {
    setPlayers((prev) => {
      ...
      const newMax = Math.max(1, Math.floor(next.length / 4));
      setNumberOfCourts((c) => Math.min(c, newMax)); // clamp down
      return next;
    });
  }
  ...
  function generateMatchups(
    ...
    while (usedIds.size < pool.length) {
      ...
      let roundPlayers =
        notYetPlayed.length >= playersPerRound
          ? notYetPlayed.slice(0, playersPerRound)
          : [
              ...notYetPlayed,
              ...fillIns.slice(0, playersPerRound - notYetPlayed.length),
            ];
      ...
      if (roundPlayers.length > targetCount) {
        roundPlayers.sort(
          (a, b) => sitOutCount.get(a.id) - sitOutCount.get(b.id),
        );
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        candidateTeams.sort((a, b) => {
          const scoreA =
            skillClass[a.player1.skill] + skillClass[a.player2.skill];
          const scoreB =
            skillClass[b.player1.skill] + skillClass[b.player2.skill];
        ...
        for (let i = 0; i < candidateTeams.length; i++) {
          ...
          for (let j = i + 1; j < candidateTeams.length; j++) {
            ...
            const skillDiff = Math.abs(
              skillClass[candidateTeams[i].player1.skill] +
                skillClass[candidateTeams[i].player2.skill] -
                (skillClass[candidateTeams[j].player1.skill] +
                  skillClass[candidateTeams[j].player2.skill]),
            );

            let repeats = 0;
            const aIds = [
              candidateTeams[i].player1.id,
              candidateTeams[i].player2.id,
            ];
            const bIds = [
              candidateTeams[j].player1.id,
              candidateTeams[j].player2.id,
            ];
        ...
        const maxDiff = candidateCourts.reduce((max, court) => {
          const diff = Math.abs(
            skillClass[court.teamA.player1.skill] +
              skillClass[court.teamA.player2.skill] -
              (skillClass[court.teamB.player1.skill] +
                skillClass[court.teamB.player2.skill]),
          );
  ...
  return (
    <div>
      ...
      <p className="intro-text">
        Enter players, balance skill levels, and generate fair matchups.
      </p>
